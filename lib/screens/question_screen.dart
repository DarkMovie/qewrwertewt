...
class QuestionScreen extends StatefulWidget {
  final String testType; // 'random' or 'booklet'
  final int? questionCount; // تعداد سوالات برای تست تصادفی

  const QuestionScreen({super.key, required this.testType, this.questionCount});
...
class _QuestionScreenState extends State<QuestionScreen>
  ...
  void initState() {
    ...
    if (widget.testType == 'random') {
      // اگر تعداد سوالات مشخص شده باشد از آن استفاده کن، در غیر این صورت از 20 به عنوان پیش‌فرض استفاده کن
      _questionCount = widget.questionCount ?? 20;
    } else {
      _questionCount = 0; // Will be set later
    }
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (widget.testType == 'random') {
        testProvider.startRandomTest(_questionCount);
      } else {
        // For booklet tests, we'll use all questions from the booklet
        testProvider.startBookletTest(
          int.parse(widget.testType.replaceAll('booklet', '')),
        );
        _questionCount = testProvider.currentQuestions.length;
      }
  ...
  Future<void> _finishTest(TestProvider testProvider) async {
    ...
    try {
      await testProvider.saveTestResult(widget.testType);
    ...
    return Consumer<TestProvider>(
      builder: (context, testProvider, child) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(
              widget.testType == 'random'
                  ? 'تست تصادفی ($_questionCount سوال)'
                  : 'دفترچه ${widget.testType.replaceAll('booklet', '')}',
